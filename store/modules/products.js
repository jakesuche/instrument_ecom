...
const getters = {
  ...
  changeCurrency: (state) => {
    if (state.currency.curr === "eur") {
      ...
      return state.currency;
    } else if (state.currency.curr === "ngn") {
      state.currency.curr = 410;
      return state.currency;
...
const actions = {
  ...
  getAllproducts({commit}){
    ...
    this.$api.$get('/api/niconkart/products')
    .then((res)=>{
      console.log(res, 'this is prdocus')
      commit('storeProduct', res.products)
    })
    .catch((err)=>{
      console.log(err)
    })
